...
export default function Toolbar(props: any) {
	...
	return (
		...
		>
			...
			<div>
				<div className={` h-[100%] w-[100%] ${selectedBlocks.some((selectedBlock) => selectedBlock.id === props.block.id) ? "absolute opacity-50 bg-blue-100 rounded-lg" : "hidden"} ${blockRadius}`} />
					...
					<ClickOutsideListener callback={handleToolbarClose}>
						<div className="fixed flex rounded-[5px] p-1 shadow bg-[white] z-50 -translate-x-[100%]" style={{ top: toolbarTop, left: toolbarLeft }} ref={toolbarRef}>
							<Grid numberOfCols={1}>
								...
								<Item text="Delete block" tabFocus={false} textColor="red" icon={DeleteIcon} action={deleteBlock} />
				...
				createPortal(
					<ClickOutsideListener callback={handleEditorClose}>
						<div className="fixed flex rounded-[5px] p-1 shadow bg-[white] z-50" style={{ top: editorTop, left: editorLeft }} ref={editorRef}>
							<editor.editor id={props.id} tabFocus={false} block={props.block} attribute={editor.key} value={props.block[editor.key]} extraProps={editor.extraProps} />
						</div>
					</ClickOutsideListener>,
					document.body,
				)}
